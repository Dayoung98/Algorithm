...
public class Solution {
	...
	static ArrayList<int []> list; //고려해야하는 코어만 담고있는 리스트 (가장자리는 제외)
	...
	private static boolean isAvailable(int r,int c,int d) {
		...
		while(true) {
			nr +=dr[d];
			nc +=dr[d];
			if(nr<0 || nr>=N || nc<0 || nc>=N) break; //계속해서 해당방향으로 직진해서 경계를 벗어남 -> 전원 연결 가능
	...
	private static void setStatus(int r,int c, int d,int status) {
		...
		while(true) {
			...
			map[nr][nc] = status; //현재 상태 값으로 셋팅하기
		}
	}
